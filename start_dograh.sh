...
export COMPOSE_PROJECT_NAME=dograh
...
sync_postgres_password() {
    ...
    REGISTRY="$REGISTRY" ENABLE_TELEMETRY="$ENABLE_TELEMETRY" docker-compose -f dograh-docker-compose.yml up -d postgres
    ...
    printf '%s\n' "ALTER USER postgres WITH PASSWORD :'dograh_password';" \
        | docker-compose -f dograh-docker-compose.yml exec -T postgres psql \
        -U postgres \
        -d postgres \
        -v ON_ERROR_STOP=1 \
...
[[ -f dograh-docker-compose.yml ]] || fail "dograh-docker-compose.yml not found. Download it first, then re-run this script."
...
echo ""
echo "This will run:"
echo "  REGISTRY=$REGISTRY ENABLE_TELEMETRY=$ENABLE_TELEMETRY docker-compose -f dograh-docker-compose.yml --profile tunnel up -d --pull always"
echo ""
...
REGISTRY="$REGISTRY" ENABLE_TELEMETRY="$ENABLE_TELEMETRY" docker-compose -f dograh-docker-compose.yml --profile tunnel up -d --pull always
