...
if len(faculty_names) < 150: faculty_names += [f"Prof. {i}" for i in range(150)]
subjects = list(set(subjects))
if len(subjects) < 150: subjects += [f"Subject {i}" for i in range(150)]

departments = ["Computer Science and Engineering", "Information Technology", "Mechanical Engineering", "Civil Engineering", "Electrical Engineering", "Electronics and Communication", "MCA", "MBA", "Applied Sciences", "Production Engineering", "Computer Applications", "Business Administration"]
degrees = ["B.Tech", "M.Tech", "MCA", "MBA", "PhD", "B.Sc", "M.Sc", "BBA", "Diploma"]
semesters = ["1st", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "first", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth", "final", "penultimate"]
student_categories = ["SC/ST", "OBC", "General", "Sikh Minority", "Rural", "Management Quota", "Defense Personnel", "EWS", "Sports Quota", "J&K Resident"]
fees_aspects = ["hostel fee", "tuition fee", "mess fee", "total fee", "security deposit", "library fee", "exam fee", "transport fee", "alumni fee", "development fund", "registration fee"]
companies = ["TCS", "Infosys", "Wipro", "Cognizant", "Accenture", "Microsoft", "Google", "Amazon", "Tech Mahindra", "IBM", "Capgemini", "Samsung", "Trident", "Hero Cycles", "Vardhman", "L&T", "Reliance", "Tata Motors", "Mahindra"]
designations = ["Professor", "Associate Professor", "Assistant Professor", "Guest Faculty", "Adhoc Faculty", "Visiting Faculty", "Lecturer"]
qualifications = ["PhD", "Ph.D.", "Masters", "M.Tech", "Postdoc", "M.Sc", "M.E."]
...
templates = [
    ...
    "Is entrance exam required for {degree} in {dept}?",
    "What is the intake capacity for {degree} {dept}?",
    "Do you have a management quota for {degree} {dept}?",
    ...
    "When is the last date to pay the {fee} for {sem} semester?",
    "Are there any late fines on {fee}?",
    ...
    "Is {subject} an elective or compulsory in {dept}?",
    "What is the evaluation criteria for {subject}?",
    ...
    "Does {faculty} take {degree} classes?",
    "What subjects does {faculty} teach?",
    
    # Faculty Aggregates & Statistics
    ...
    "What percentage of faculty in {dept} has a {qual}?",
    "Are there more than 10 {designation}s in {dept}?",
    ...
    "What is the minimum CGPA required for {company}?",
    "Did {company} hire from {dept} this year?",
    "What role did {company} offer to {degree} students?"
]

prefixes = ["", "Could you tell me, ", "I want to know: ", "Please explain: ", "Do you have information on ", "Can you answer this: ", "Hey bot, ", "Hi, ", "Greetings, ", "Excuse me, ", "I have a question: ", "Can anyone tell me, ", "Just wondering, ", "I was wondering, ", "Would you know, "]
suffixes = ["", " Thanks.", " Please provide details.", " Any help is appreciated.", " Quickly please.", " Thanks in advance.", " Much appreciated.", " Regards.", " If possible.", " Let me know.", " I need this urgently."]

questions = set()
attempts = 0
max_attempts = 3000000

while len(questions) < 500000 and attempts < max_attempts:
    attempts += 1
    ...
    if random.random() > 0.4:
        ...
        if q[0].isalpha() and pref:
            q = pref + q[0].lower() + q[1:]
        else:
            q = pref + q
...
output_file = "data/500000_rag_questions.txt"

# Writing in chunks to be safe with memory though it should be fine
